...
import { RelicManager } from '../managers/RelicManager.js'; // <-- ИМПОРТ
...
export class ShopScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Инициализируем менеджер реликвий для этой сцены
        ...
        this.dimmer = this.add.rectangle(0, 0, GW, GH, 0x000000, 0.85).setOrigin(0).setVisible(false).setDepth(900).setInteractive();
    ...
    generateRelic(GW, GH) {
        ...
        priceTag.on('pointerdown', () => {
            if (GameState.gold >= price) {
                if (GameState.relics.includes(randomKey)) { /* Можно добавить проверку */ }

                GameState.gold -= price;
                
                // ВАЖНО: Используем менеджер для добавления!
                // Это запустит триггер onPickup (Клубника сработает тут)
                this.relicManager.addRelic(randomKey); 
    ...
    unzoomCard() {
        ...
        this.tweens.add({ targets: card, x: card.savedX, y: card.savedY, scale: 1, duration: 250, ease: 'Power2', onComplete: () => { card.setDepth(0); } });
    }
}

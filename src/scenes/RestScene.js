...
export class RestScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.tweens.add({
            targets: this.add.text(GW/2, GH/2 - 80, "🔥", { fontSize: '120px' }).setOrigin(0.5),
            scale: 1.1, duration: 1000, yoyo: true, repeat: -1
        });
    ...
    openCardSelection() {
        ...
        const title = this.add.text(GW/2, 60, "SELECT A CARD TO ENCHANT", { fontSize: '32px', color: '#ffaa00' }).setOrigin(0.5);

        // Показываем карты, у которых есть свободные слоты (< 2 зачарований)
        ...
        validCards.forEach((cardInstance, index) => {
            ...
            // КЛИК ПО КАРТЕ -> ПЕРЕХОД К ВЫБОРУ ЗАЧАРОВАНИЯ
            card.bg.setInteractive();
            card.bg.on('pointerdown', () => {
                this.showEnchantOptions(cardInstance);
            });
        ...
        // Кнопка отмены
        const cancelBtn = this.add.text(GW/2, GH - 50, "[ CANCEL ]", { fontSize: '24px', color: '#fff' })
            .setOrigin(0.5).setInteractive();
        cancelBtn.on('pointerdown', () => this.scene.restart());
    }

    // --- ШАГ 2: ВЫБОР ЗАЧАРОВАНИЯ (DRAFT) ---
    showEnchantOptions(cardInstance) {
        // Очищаем список карт, оставляем фон
        ...
        // Показываем выбранную карту в центре (чуть выше)
        ...
        // Генерируем 3 варианта с учетом редкости
        const options = this.rollEnchants(3);

        const startX = GW/2 - 350;
        const gap = 350;
        ...
        options.forEach((enchantKey, index) => {
            ...
            let color = 0x666666; // Common
            if (data.rarity === 'rare') color = 0x0088ff;
            if (data.rarity === 'legendary') color = 0xffaa00;
            ...
            // Клик по варианту -> Применить и выйти
            bg.on('pointerdown', () => {
    ...
    // Логика рандома с весами (как у реликвий)
    rollEnchants(count) {
        const keys = [];
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (rand < 0.10) selectedRarity = 'legendary';      // 10%
            else if (rand < 0.40) selectedRarity = 'rare';      // 30%
            
            // Если в редком пуле пусто, берем обычный
